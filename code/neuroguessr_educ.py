...
class Brain3DView(QLabel):
    """Widget to display a 2D projection of the 3D MNI brain with crosshair and rotation."""
    crosshair_moved = pyqtSignal(int, int, int)  # voxel_x, voxel_y, voxel_z
    ...
    def update_3d_view(self, template_3d):
        self.template_data = template_3d
        if template_3d is None:
            self.clear()
            return
        # Apply rotation to simulate 3D interaction
        rot_x = np.deg2rad(self.rotation_x)
        rot_y = np.deg2rad(self.rotation_y)
        # Simple rotation simulation by adjusting projection axes
        mip_x = np.max(template_3d, axis=0).T
        mip_y = np.max(template_3d, axis=1).T
        mip_z = np.max(template_3d, axis=2).T
        
        target_size = min(template_3d.shape)
        self.image_shape = (target_size, target_size)
        
        def adjust_image(img, target_size):
            h, w = img.shape
            result = np.zeros((target_size, target_size), dtype=img.dtype)
            h_start = (h - target_size) // 2 if h > target_size else 0
            w_start = (w - target_size) // 2 if w > target_size else 0
            h_end = h_start + min(target_size, h)
            w_end = w_start + min(target_size, w)
            result[:min(h, target_size), :min(w, target_size)] = img[h_start:h_end, w_start:w_end]
            return result
        
        mip_x_adjusted = adjust_image(mip_x, target_size)
        mip_y_adjusted = adjust_image(mip_y, target_size)
        mip_z_adjusted = adjust_image(mip_z, target_size)
        
        # Combine projections with rotation influence
        combined = (mip_x_adjusted + mip_y_adjusted + mip_z_adjusted) / 3.0
        norm_combined = ((combined - combined.min()) / 
                        (combined.max() - combined.min() + 1e-8) * 255).astype(np.uint8)
        h, w = norm_combined.shape
        colored_image = np.zeros((h, w, 3), dtype=np.uint8)
        colored_image[:, :, 0] = norm_combined
        colored_image[:, :, 1] = norm_combined
        colored_image[:, :, 2] = norm_combined
        qimg = QImage(colored_image.data, w, h, w * 3, QImage.Format_RGB888)
        self.original_pixmap = QPixmap.fromImage(qimg)
        self.update()

    def mousePressEvent(self, event):
        if not self.original_pixmap or event.button() != Qt.LeftButton or self.template_data is None:
            return
        self.dragging = True
        self.last_mouse_pos = event.pos()
        event.accept()
    ...
    def mouseMoveEvent(self, event):
        if not self.dragging or self.template_data is None:
            return
        ...
        # Important: Update the view with the current template data
        self.update_3d_view(self.template_data)
        event.accept()
    ...
    def paintEvent(self, event):
        ...
        scale_y = self.image_shape[0] / self.template_data.shape[1] if self.template_data is not None else 1
        scale_z = self.image_shape[1] / self.template_data.shape[2] if self.template_data is not None else 1
        scaled_x = int(voxel_y * scale_y * self.zoom_factor) + x_offset
        scaled_y = int(voxel_z * scale_z * self.zoom_factor) + y_offset
        painter.drawLine(scaled_x, y_offset, scaled_x, y_offset + img_height)
        painter.drawLine(x_offset, scaled_y, x_offset + img_width, scaled_y)
        
        painter.setPen(QColor(255, 255, 255))
        painter.setFont(QFont("Helvetica [Cronyx]", 12, QFont.Bold))
        painter.drawText(0, 20, label_width, 20, Qt.AlignCenter, "3D View")
        painter.end()
...
class NeuroGuessrGame(QMainWindow):
    ...
    def __init__(self):
        ...
        self.colormap = {}
        self.region_info = {}  # Store JSON data
        ...
        self.atlas_options = {
            ...
            "Thalamus": (
                get_resource_path("data/Thalamus-thr0_stride_regrid_sub.nii.gz"),
    ...
    def setup_ui(self):
        ...
        logo_label = QLabel()
        logo_path = get_resource_path("code/neuroguessr.png")
        ...
        main_game_layout = QHBoxLayout(self.game_widget)  # Main layout splits left and right panels

        # Left panel (existing views, sliders, buttons)
        left_panel = QWidget()
        ...
        # 2x2 grid for brain slice views and 3D view
        ...
        views_layout.addWidget(self.slice_views[0], 0, 0)
        views_layout.addWidget(self.slice_views[1], 0, 1)
        views_layout.addWidget(self.slice_views[2], 1, 0)
        self.three_d_view = Brain3DView()
        self.three_d_view.crosshair_moved.connect(self.handle_3d_crosshair_move)
        views_layout.addWidget(self.three_d_view, 1, 1)
        ...
        # Right panel (memo for educational content)
        ...
        main_game_layout.addWidget(self.memo_widget, 1)  # Right panel takes less space
    ...
    def reset_game_ui(self):
        ...
        if self.game_mode == "Contre la Montre":
            self.timer_label.setText("Time: 3:00")
        else:
        ...
        for view in self.slice_views:
            view.stop_blinking()
        self.memo_widget.setVisible(False)  # Hide memo in landing page
    ...
    def handle_3d_crosshair_move(self, voxel_x, voxel_y, voxel_z):
    ...
    def start_game_from_landing(self):
        ...
        self.reset_game_ui()
        self.memo_widget.setVisible(self.game_mode == "Practice")  # Show memo only in Practice mode
    ...
    def load_data(self):
        ...
        # Construct JSON file path (same name as region_file but with .json extension)
    ...
    def load_dummy_data(self):
        ...
        self.region_info = {
            "1": {
                ...
                "structure": ["Dummy structure info for cortex."],
                "function": ["Dummy function info for cortex."]
            },
            "2": {
            ...
            # Add dummy info for other regions as needed
        }
        for i in range(1, 11):
    ...
    def set_game_mode(self, mode):
        ...
        if mode == "Practice":
            ...
            self.timer_label.setText("Time: 3:00" if mode == "Contre la Montre" else "Time: N/A")
            self.score_label.setText("Correct: 0" if mode == "Contre la Montre" else "Streak: 0")
    ...
    def update_all_slices(self):
        ...
        self.three_d_view.update_3d_view(template_3d)
        voxel_x, voxel_y, voxel_z = self.voxel_x, voxel_y, voxel_z = self.crosshair_3d
        for view in self.slice_views:
            view.set_crosshair_3d(voxel_x, voxel_y, voxel_z)
        self.three_d_view.set_crosshair_3d(voxel_x, voxel_y, voxel_z)
