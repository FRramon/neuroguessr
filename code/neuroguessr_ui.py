...
# mettre brodmann au bonne dimension
# verif MNI pour HO et Thalamus
...
class BrainSliceView(QLabel):
    ...
    def update_slice(self, slice_data, template_slice, colormap=None):
        """Update the displayed brain slice with template as background."""
        self.slice_data = slice_data
        self.template_data = template_slice
        if slice_data is None or template_slice is None:
            self.clear()
            return
        # Normalize template data for grayscale background
        ...
        # Create RGB image starting with template as grayscale
        ...
        # Overlay atlas with transparency
        if colormap:
            unique_vals = np.unique(slice_data)
            for val in unique_vals:
                if val in colormap and val > 0:  # Skip background
                    mask = (slice_data == val)
                    color = colormap[val]
                    # Blend atlas color with background (50% opacity)
...
# [BrainSliceView class remains unchanged, as it handles slice display and interaction]

class NeuroGuessrGame(QMainWindow):
    """Main window for the NeuroGuessr game with chronometer mode."""
    ...
    def __init__(self):
        ...
        self.correct_guesses = []  # Track correct regions
        self.incorrect_guesses = []  # Track incorrect guesses
    ...
    def setup_ui(self):
        ...
        self.score_label = QLabel(f"Correct: {self.score}")
    ...
    def load_data(self):
        """Load brain data, template, and region mapping."""
        ...
        try:
            ...
            if os.path.exists(atlas_file) and os.path.exists(region_file):
                ...
                # Verify template and atlas compatibility
    ...
    def start_game(self):
        """Start the game in chronometer mode."""
        self.score = 0
        self.errors = 0
    ...
    def update_all_slices(self):
        """Update all slice views with template and atlas."""
        # [Unchanged from original]
    ...
    def validate_guess(self):
        ...
        self.guess_button.setEnabled(False)
        self.guess_button.setStyleSheet("font-size: 16px; padding: 10px; background-color: #4CAF50;")
        # Immediately select a new target
        self.select_new_target()
