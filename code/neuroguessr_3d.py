...
class Brain3DView(QLabel):
    """Widget to display a 3D brain mesh loaded from an OBJ file with crosshair and rotation."""
    ...
    def __init__(self, parent=None):
        ...
        # Create Qt3DWindow and embed it
        ...
        # Axes
    def set_crosshair_3d(self, voxel_x, voxel_y, voxel_z):
        """Set the 3D crosshair position and update the view."""
        self.crosshair_3d = (voxel_x, voxel_y, voxel_z)
        if self.template_data is not None:
            pos = np.array([voxel_x, voxel_y, voxel_z]) - self.brain_center
    ...
    def wheelEvent(self, event):
        ...
        if delta > 0:
            ...
            self.zoom_factor /= 1.2
        self.zoom_factor = max(0.5, min(self.zoom_factor, 5.0))
        self.camera.setPosition(QVector3D(0, 0, 5 / self.zoom_factor))
        self.update()
        event.accept()

    def update_3d_view(self, template_3d):
        """Update the 3D view with the brain mesh based on the current atlas."""
        self.template_data = template_3d
        if template_3d is None:
            return

        # Clear existing mesh
        for child in self.root_entity.childNodes():
            if hasattr(child, 'isMesh') and child.isMesh:
                child.setParent(None)

        # Determine which OBJ file to load based on atlas
        obj_file = get_resource_path("data/xtract.obj" if self.current_atlas == "Xtract" else "data/temp_mri_mesh.obj")
        ...
        mesh = QMesh()
        mesh.setSource(QUrl.fromLocalFile(obj_file))
        brain_entity.isMesh = True  # Mark as mesh for cleanup

        # Material
        material = QPhongMaterial()
        material.setAmbient(QColor(204, 204, 204))  # Light gray
        material.setDiffuse(QColor(204, 204, 204, 255))  # Fully opaque

        # Apply transform
        dims = template_3d.shape
        self.brain_center = np.array(dims) / 2
        max_extent = max(dims) / 2
        self.scale_factor = 2.0 / max_extent if max_extent != 0 else 1.0
        self.transform.setScale(self.scale_factor)
        self.transform.setTranslation(QVector3D(0, 0, 0))

        brain_entity.addComponent(mesh)
        brain_entity.addComponent(material)
        brain_entity.addComponent(self.transform)

        # Update crosshair if it exists
        if self.crosshair_3d != (0, 0, 0):
            self.set_crosshair_3d(*self.crosshair_3d)

        self.update()

    def mousePressEvent(self, event):
        """Handle mouse press events to initiate dragging or set crosshair."""
        if event.button() != Qt.LeftButton or self.template_data is None:
            return
        self.dragging = True
        self.last_mouse_pos = event.pos()
        if self.mode == 'move':
            center = self.brain_center
            self.crosshair_moved.emit(int(center[0]), int(center[1]), int(center[2]))
            self.set_crosshair_3d(int(center[0]), int(center[1]), int(center[2]))
        event.accept()

    def mouseMoveEvent(self, event):
        """Handle mouse move events for rotation or movement."""
    ...
    def paintEvent(self, event):
        """Custom paint event to draw the '3D View' label."""
        super().paintEvent(event)
        painter = QPainter(self)
        painter.setPen(QColor(255, 255, 255))
        painter.setFont(QFont("Helvetica [Cronyx]", 12, QFont.Bold))
        painter.drawText(0, 20, self.width(), 20, Qt.AlignCenter, "3D View")
        painter.end()

    def set_atlas(self, atlas_name):
        """Set the current atlas to determine which OBJ file to load."""
        self.current_atlas = atlas_name
        if self.template_data is not None:
            self.update_3d_view(self.template_3d)

    def set_mode(self, mode):
        """Set interaction mode ('rotate' or 'move')."""
        self.mode = mode
...
class NeuroGuessrGame(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.quit_button.setStyleSheet("""
            QPushButton {font-size: 16px; padding: 10px; background-color: #f44336; color: white; border-radius: 5px;}
            QPushButton:hover {background-color: #d32f2f;}
        """)
    ...
    def start_game_from_landing(self):
        ...
        self.three_d_view.set_atlas(self.current_atlas)  # Update atlas in 3D view
        self.load_data()
